...
console.log(
  chalk.green.italic.bold(
    `\t\t        "WELLCOME TO STUDENT EDUCATION SYSTEM APP"`
  )
);
console.log(
  chalk.yellow(
...
while (userInput.userName.length < 4) {
  ...
  console.log(
    chalk.red.italic.bold`\t\t         NAME MUST BE AT LEAST 4 CHARACTERS LONG`
  );
  console.log(
    chalk.yellow(
      `\t\t============================================================\n`
...
let userInput_2 = await inquirer.prompt([
  {
    name: "userRollNumber",
    message:
      "ENTER YOUR ROLL NUMBER. MAKE SURE YOUR ROLL NUMBER STARTS WITHOUT 0:",
    type: "string",
  },
]);
...
if (input_2.requirements === "PAY COARSE FEES") {
  ...
  if (
    (input_2.courses === "CLOUD APPLIED GENERATIVE AI: FEES => 15000" &&
      payment.money === 15000) ||
    ...
    if (userInfo.balance >= payment.money) {
      ...
      console.log(
        chalk.green.italic.bold(`\t\t\t      PAYMENT IS SUCCESS FULLY PAID`)
      );
      console.log(
        chalk.yellow(
          `\t\t    ==================================================\n`
      ...
      console.log(
        chalk.red.italic.bold(
          `\t\t    AFTER DEPOSITING THE FEES YOUR BALANCE IS THIS: ${
            userInfo.balance - payment.money
          }`
        )
      );
      console.log(
      ...
      console.log(
        chalk.red.italic.bold(`\t\t\t   SORRY YOU DON'T HAVE ENOUGH MONEY.`)
      );
      console.log(
        chalk.red.italic.bold(
          `\t\t\t         YOUR BALANCE IS: ` + userInfo.balance
        )
      );
      console.log(
        chalk.yellow(
  ...
  console.log(
    chalk.green.italic.bold(
      `HERE'S YOUR STATUS MEANS YOUR INFORMATION ONES YOU PROVIDED US IN INPUT.`
    )
  );
  console.log(
    chalk.yellow(
  ...
  console.log(
    chalk.green.italic.bold(`\t\t     THANKS FOR USING CREATOR BY :`)
  );
