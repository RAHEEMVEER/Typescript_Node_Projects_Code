...
class UserAccounts extends Bank {
    ...
    createUserAccount(addUser) {
        accounts.push(addUser);
    }
    removeUserAccount(remUser) {
        const userIndex = accounts.findIndex((user) => user.accountNumber === remUser.accountNumber);
        if (userIndex !== -1) {
...
class User extends Bank {
    ...
    gender;
    balance = 0;
    constructor(name, age, cnicNum, gender, accountNumber) {
        super(accountNumber);
...
async function main() {
    async function addUserBank() {
        ...
        const user = new User(userDetail.Name, userDetail.age, userDetail.cnic, userDetail.gender, randomAccountNumber);
    ...
    async function viewAllAccounts() {
        if (accounts.length === 0) {
            ...
            for (let details of accounts) {
                console.log(chalk.green.bold.italic(`\n\t\tPERSON NAME IS: ${details.name.toUpperCase()}\n\t\tTHEIR AGE IS: ${details.age}\n\t\tTHEIR CNIC NUMBER IS: ${details.cnicNumber}\n\t\tTHEIR GENDER IS: ${details.gender}\n\t\tTHEIR ACCOUNT NUMBER IS: ${details.accountNumber}\n\t\tTHEIR BALANCE IS: ${details.balance}\n`));
            }
            ...
            if (foundSpecificAccount.specific) {
                ...
                let foundUser = accounts.find((user) => user.accountNumber === viewUser.view);
                if (foundUser) {
    ...
    async function removeUserBank() {
        if (accounts.length === 0) {
            ...
            let remUser = accounts.find((user) => user.accountNumber === remAccountNumber.removeAccount);
            if (remUser) {
                const account = new UserAccounts(remUser.accountNumber);
                account.removeUserAccount(remUser);
                userAccountNumbers.splice(userAccountNumbers.indexOf(remUser.accountNumber), 1);
            }
        }
    }
    async function depositMoney() {
        ...
        if (userAccountNumbers.length === 0) {
            ...
            let userAccount = accounts.find((acc) => acc.accountNumber === depositFromAccount.accNum);
            if (userAccount) {
    ...
    async function withdrawMoney() {
        ...
        if (userAccountNumbers.length === 0) {
            ...
            let userAccount = accounts.find((acc) => acc.accountNumber === withdrawFromAccount.accNum);
            if (userAccount) {
