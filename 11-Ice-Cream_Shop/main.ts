...
async function icecreamShop() {
  ...
  while (dry.dry > items.dryFruits.length - 1) {
    console.log(
      chalk.red.bold("\tPLEASE TYPE VALID DRY FRUITS ITEM NUMBER.\n")
    );
  ...
  return new Promise((resolve, reject) => {
    console.log(
      chalk.yellow.bold("THE ICE-CREAM YOU ORDERED IS BEING PREPARING....\n")
    );
    let isShop = true;
    if (isShop) {
      setTimeout(() => {
        console.log(
          chalk.blue.bold.italic(`ADD `) +
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              console.log(
                chalk.blue.bold.italic(`WE ARE PUTTING THE ICE-CREAM INTO `) +
      ...
      reject(
        chalk.red.bold.italic("THE SHOP IS CLOSED YOU WILL ALSO COME TOMORROW.")
      );
    }
  });
}
